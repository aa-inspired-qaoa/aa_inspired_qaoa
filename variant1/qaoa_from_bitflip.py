...
class QAOAbf():
    ...
    def build_circuit(self, p):
        ...
        gammas = [Parameter("G%d" % i) for i in range(0, p)]
        ...
        for i in range(0, p):
            # cost 
            circ.append(self.cost_circuit.assign_parameters([gammas[i]], inplace=False), [*qaoa_qubits, *ancilla_qubits])
            # mixer 
...
def get_cost_circuit(input_formula):
    ...
    cost_circ.p((-1)*gamma, cost_circ.num_qubits - 1) # phase gate on the result qubit
